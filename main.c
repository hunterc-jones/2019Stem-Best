...
bool usingElbow = true;		//CONFIG VAL 		set false for Kyle's prototype
int wristPos = 0;
int wristDefaultTheta = 45;
int theta = 59;					// CONFIG VAL 		to rotate servo by arc length of one wire-holder's diameter
int minWristPos = 0;		// 		no need to config as non-zero, unless claw can move backwards from starting default pos
int maxWristPos = 3;		// CONFIG VAL 		to determine the maximum amount of servo-theta increments allowed for claw
bool minusThetaWristBtnPressed = false;
bool plusThetaWristBtnPressed = false;
float elbowPot = 0.0;
bool deployElbow = false;
bool elbowCheck = true;
float deadband = 3.0;		// CONFIG VAL to reduce deadband


void SetupRobot() {
	// min 40
	// max -120
	motor[wrist] = wristDefaultTheta;
}
