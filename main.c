...
void CheckDriveModes() {
	...
	if ((vexRT[Btn8U] && !mainautoBtnPressed) && (!vexRT[Btn8D])) {
		driveMode = 2;
		mainautoBtnPressed = true;
	} else if (!vexRT[Btn8U]) {
		mainautoBtnPressed = false;
	}
	if ((vexRT[Btn8D] && !ralstonvalleyautoBtnPressed) && (vexRT[Btn8U])) {
		driveMode = 3;
...
void Arcade() {
	...
	if (testingExpo) {
		if (abs(vexRT[Ch2] - vexRT[Ch1]) > deadband) {
			motor[right] = pow(vexRT[Ch2]+vexRT[Ch1], (abs((vexRT[Ch2] + vexRT[Ch1])/127)));
		} else {
			motor[right] = 0;
		}
		if (abs(vexRT[Ch1] + vexRT[Ch2]) > deadband) {
			motor[left] = pow(vexRT[Ch1]+vexRT[Ch2], (abs((vexRT[Ch1] + vexRT[Ch2])/127)));
		} else {
			motor[left] = 0;
		}
	} else {
...
void MainAuto() {
	...
	if (!StartingPathFound) {
		//motor[right] = rightMotorPathFindSpeed;
		//motor[left] = leftMotorPathFindSpeed;
